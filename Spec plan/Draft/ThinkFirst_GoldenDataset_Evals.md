...
| `strike_count` resets at Dim 2 | Yes — back to 0 |
| Output brief with all weak dims | Generates at Turn 12 — honest, not inflated |
| Brief tone with all weak dims | Honest about gaps, not discouraging |

**Key check note:** An output brief must still generate at Turn 12 even with all weak dimensions. Honest output — not fake positivity.
...
Open the system prompt from Prompt & Policy Spec v2. Paste T1's opening input. Run each turn manually. Copy the raw JSON per turn into your JSON Check Log sheet.
...
*ThinkFirst · Golden Dataset & Eval Framework v1.0 · April 2026*
