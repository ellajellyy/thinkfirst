...
**Version:** 3.0
**Date:** April 2026
**Status:** Ready for Retest — Round 3
**Stage:** Phase 2 — Shape the Product

**Changelog v3.0:**
- Fix 1 (Critical): Added explicit one-sentence clarification to schema rules: output_ready=false and next_action=trigger_output do NOT need to match — they are two separate systems. This resolves the tester confusion flagged in I02/I09/I10 where "resolved" was marked but the contradiction remained in the spec.
- Fix 2 (Medium): Added concrete evidence thresholds to the ok/strong boundary for all 4 dimensions. Each dimension now has a specific example of what counts as ok versus strong — not just a description.
- Fix 3 (Medium): Added consistency rule — the same answer must score the same way regardless of which turn it appears in. Dimension scores cannot drift backward without new contradicting evidence from the builder.
...
IMPORTANT — Consistency rule: A dimension score can only move forward, never backward without new contradicting evidence. If user_specificity was scored "ok" on Turn 2, it cannot drop back to "weak" on Turn 5 unless the builder has said something that directly contradicts or retracts what they said before. The same answer must score the same way regardless of which turn it appears in. Scoring cannot drift based on turn number or conversation length.
...
- If turn_count >= 12 regardless of dimension state:
  next_action = "trigger_output"
  → Hard cap. Do not ask another question under any circumstances. Turn 12 is the absolute final turn — there is no Turn 13. Set next_action="trigger_output" immediately.
...
- output_ready is ALWAYS false in Claude's JSON — every single turn, no exceptions. Claude never sets this to true. This is correct and intentional.
- next_action="trigger_output" and output_ready=false do NOT need to match — they are two completely separate systems. trigger_output is the only signal Claude sends to end the conversation. The backend reads that signal and sets output_ready=true on its own. Claude's JSON having output_ready=false at the same time as next_action="trigger_output" is the correct and expected state — not a bug.
...
| turn_count >= 12 | Force output — no exceptions | Output Generation Prompt with gaps flagged. Claude must not send Turn 13. |

Turn count hard cap: **12 turns maximum.** If trigger_output has not fired by turn_count=12, Claude must set next_action="trigger_output" on Turn 12 itself — not Turn 13. The conversation ends at Turn 12 regardless of dimension state or strike count. There is no Turn 13.
...
| **Hard cap** | turn_count = 12 | Claude sets next_action="trigger_output" on this turn. No Turn 13 under any circumstances. |
